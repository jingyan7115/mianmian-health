(() => {
  ...
  async function loadCloudData({ notify = false } = {}) {
    setCloudStatus("loading", "正在读取最新线上数据");
    let apiError = null;
    ...
    overlay.innerHTML = `<div class="modal">
      <section class="modal-card">
        ...
        <form class="modal-body" id="editor-form">
          <div class="privacy">保存后会写入线上健康库，并同步到同一个公开链接。医学结论和剂量请按原始报告或医嘱准确录入。</div>
          <div style="height:14px"></div>
          <div class="form-grid">
      ...
      try {
        ...
        for (const file of formData.getAll("files") || []) {
          if (file instanceof File && file.size) {
            submit.textContent = `正在上传 ${file.name}…`;
            imageUrls.push(await uploadCloudAsset(file));
          }
        }
    ...
    overlay.innerHTML = `<div class="modal">
      <section class="modal-card">
        ...
        <form class="modal-body" id="upload-form">
          <div class="privacy">扫描件和你核对后的指标会保存到线上，收到公开链接的人可查看。页面不会自行生成医学结论；请按照原报告逐项核对数值、单位和异常状态。</div>
          <div style="height:14px"></div>
          <div class="form-grid">
    ...
    overlay.querySelector("#upload-form").onsubmit = async (event) => {
      ...
      try {
        ...
        for (let index = 0; index < files.length; index += 1) {
          ...
          uploaded.push({
            file: files[index],
            url: await uploadCloudAsset(files[index]),
          });
        }
